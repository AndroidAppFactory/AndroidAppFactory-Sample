...
object PuzzleGameManager {
    interface GameStateListener {
        ...
        fun onGameModeChanged(puzzleGameMode: PuzzleGameMode)
        fun onGameSuccess(level: Int, puzzleGameMode: PuzzleGameMode)
    }
    ...
    fun getGameMode(): PuzzleGameMode {
        return mPuzzleGame?.gameMode ?: PuzzleGameMode.BAD
    }

    fun changeGameMode(puzzleGameMode: PuzzleGameMode) {
        mPuzzleGame?.changeMode(puzzleGameMode)
        notifyGameModeChanged(getGameMode())
    }


    private fun notifySuccess(level: Int, puzzleGameMode: PuzzleGameMode) {
        ThreadManager.getInstance().start {
            mGameListenerList.forEach {
                it.onGameSuccess(level, puzzleGameMode)
            }
        }
    }
    ...
    private fun notifyGameModeChanged(puzzleGameMode: PuzzleGameMode) {
        ThreadManager.getInstance().start {
            mGameListenerList.forEach {
                it.onGameModeChanged(puzzleGameMode)
            }
        }
    }
}